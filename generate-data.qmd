...
> Because webR runs in the browser, it’s subject to the browser’s security restrictions. That means no socket connections, which is probably how R wants to load that URL when you do `load(url(...))`.

The `webR` version of R has a version of `download.file()` that uses something called an [XHR request](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest) and requires the right kinds of headers to come from the source. 

For GitHub, there are "raw" URLs that fit that requirement. For my repo, we can use something like: 
